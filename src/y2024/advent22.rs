...
impl Solve for Advent {
    ...
    fn compute_part1_answer(&self, test_mode: bool) -> Result<String, String>{
        ...
        let result: usize = self.numbers.iter().map(|n|{
            ...
            let mut cnt = 0usize;
            while cnt<2000{
                sn = operation(operation(operation(sn, Operation::Mult64),Operation::Div32),Operation::Mult2048);
                cnt+=1;
            }
            sn
        }).sum();
...
fn operation(secret_number: usize, operation: Operation)->usize{
    prune(mix(secret_number, match operation{
        Operation::Mult64 => secret_number<<6,
        Operation::Div32 => secret_number>>5,
        Operation::Mult2048 =>secret_number<<11
    }))
}