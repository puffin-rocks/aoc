...
impl Solve for Advent {
    ...
    fn compute_part2_answer(&self, _test_mode: bool) -> Result<String, String>{
        ...
        let mut location = 0;
        let mut skip = 0;
        while skip<64*self.numbers2.len() {
            for el in self.numbers2.iter() {
                let mut p = 0;
                let m = *el as usize;
                while p < m / 2 {
        ...
        let result = dense_hash.iter()
            .map(|num|  format!("{:02x}", num))
            .collect::<Vec<_>>()
            .join("");
        assert_display(result.as_str(), None, "35b028fe2c958793f7d5a61d07a008c8", "Knot Hash", false)
    }
}